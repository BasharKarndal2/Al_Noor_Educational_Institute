...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        // عرض مودال الإدخال
        var myModal = new bootstrap.Modal(document.getElementById('addWorkinghourModal'));
        myModal.show();
...
@php
    $routeName = session('url'); // مثلاً: 'workeing_hours.update'
@endphp
<script>
document.addEventListener('DOMContentLoaded', function () {
 
   var myModal = new bootstrap.Modal(document.getElementById('editWorkinghourModal'));
    myModal.show();
  const routeTemplate = "{{ route($routeName, ':id') }}";
        const realUrl = routeTemplate.replace(':id', "{{ session('id') }}");
        document.getElementById('editWorkinghourModalForm').setAttribute('action', realUrl);
    // عرض مودال التنبيه بالأخطاء
    const alertMessages = @json($errors->all());
    let listHtml = "";
    alertMessages.forEach(msg => {
